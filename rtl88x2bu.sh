...
echo -e "\nb\nb##########\nb\nbUpdating Package Repos & Upgrading"
apt update -y && apt upgrade -y

echo -e "\nb\nb##########\nb\nbInstalling aircrack-ng, git & dksm"
apt install aircrack-ng dkms git -y

echo -e "\nb\nb##########\nb\nbCloning repository to /usr/src/rtl88x2bu-git"
...
echo -e "\nb\nb##########\nb\nbInstalling RTL88x2BU dynamic kernel module"
sed -i 's/PACKAGE_VERSION="@PKGVER@"/PACKAGE_VERSION="git"/g' /usr/src/rtl88x2bu-git/dkms.conf
dkms add -m rtl88x2bu -v git

# \a = beep speaker!!
# \nb = linebreak hopefully
echo -e "\a\nb\nb##########\nb\nbDone with everything (hopefully)!\nYou must reboot your system and run 'dkms autoinstall' to complete install.\nbGood Luck :)"